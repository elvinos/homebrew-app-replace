...
def is_installed_by_homebrew(applications_dir, application_path):
    s = application_path.replace(applications_dir + '/', '')
    s = s.replace('.app', '')
    s = s.replace(' ', '-')
    s = s.replace('.', '')
    s = str(s).lower()
...
def is_installed_by_homebrew2(application_name):
    cmd = 'brew cask list'
    output = commands.getoutput(cmd)
    return output.find(application_name) > 0

def microsoft():
    replaceMicrosoft = False
    if is_installed_by_homebrew2('microsoft-office'):
        print('Skip Microsoft Office already installed by Homebrew')
        return replaceMicrosoft
    replace_it = input('Replace All Microsoft Office Applications (Y/n):')
...
def replace_application_in(applications_dir,
    ...
    try:
        ...
        for application in applications:
            ...
            if 'Microsoft' in application_name:
                if replaceMicrosoft:
                    replaceMicrosoft = microsoft()
                continue

            application_name = generate_cask_token(application_path,
                                                   application_name)

            if is_installed_by_homebrew2(application_name):
                print('Skip {0} already installed by Homebrew'.format(application))
                continue
